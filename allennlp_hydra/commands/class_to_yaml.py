...
@Subcommand.register("class-2-yaml")
class ClassToYAML(Subcommand):
    ...
    def add_subparser(
        ...
        description = """
        Create a YAML file for a given registrable class. Will use the
        `__init__` arguments to create the entries in the YAML file. Any
        required arguments will have values if they annotated with a type that
        has `__init__` args. Otherwise it will fill  with a `None` value.
        """
        ...
        subparser.add_argument(
            "parent_class", type=str, help="Path to the config directory."
        )
